...
# arXiv preprint arXiv:2007.02471
...
class conv_model(nn.Module):
    def __init__(self, num_layers, strides, num_channels, z_dim, num_output_channels, hidden_size, upsample_mode, act_fun,sig=None, bn_affine=True, skips=False,intermeds=None,bias=False,need_lin_comb=False,need_last=False,kernel_size=3):
        ...
        for i in range(num_layers-1):
            ...
            if i == 0:
                conv = nn.Conv2d(z_dim, num_channels, kernel_size, strides[i], padding=(kernel_size-1)//2, bias=bias)
            else:
...
def convdecoder(
    ...
    model = conv_model(num_layers, strides, num_channels, z_dim, num_output_channels, hidden_size,
